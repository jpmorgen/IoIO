...
#ap_sum_fname = '/data/io/IoIO/reduced/20200921/off_jup_ap_sum.csv'
#ap_sum_fname = '/data/io/IoIO/reduced/20200416/off_jup_ap_sum.csv'
...
with open(ap_sum_fname, newline='') as csvfile:
    ...
    for row in csvr:
        ...
        if row['EXPTIME'] < 60:
            continue
        # Skip all observations that aren't RAOFF and DECOFF observations
        if not 'Jupiter' in row['FNAME']:
            continue
        if row['LINE'] != line:
            continue
...
for id in list(set(idays)):
    ...
    this_off_jup = [rlist[i][onoff + 'Rjp50']  for i in this_day_idx]
...
plt.plot_date(mpds - pdconvert, 
              [row[onoff + 'Rjp50'] for row in median_ap_list], 's')
plt.plot_date(pds - pdconvert, 
              [row[onoff + 'Rjp50'] for row in rlist], 'k.', ms=1) #, alpha=0.2)
plt.xlabel('UT Date')
plt.ylabel(line + ' Off-Jupiter Surface Brightness (R)')
